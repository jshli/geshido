...
const convertTime = minutes => {
  hours = minutes / 60
  rest = minutes % 60
  return rest < 10  ? `${Math.floor(hours)}:0${rest}` : `${Math.floor(hours)}:${rest}`
}
...
const startTimer = event => {
  ...
  async function updateDom(event) {
    event.target.querySelector('.icn-btn').classList.add('btn--stop');
    event.target.removeEventListener('submit', startTimer);
    event.target.querySelector('.timer').textContent = await findRunningTimer(taskId).then(x => x);
            ...
            <div class='priority-wrapper'>
              ${task.priority ? `<div class="priority-div">
                ...
                <div class="task-item__header-wrap">
                  ${task.project_name!== null ? `<p class="task__project-name">${task.project_name}` : ""}
